...
async function initDb() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS bookings (
      ...
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      UNIQUE KEY unique_approved_slot (
        booking_date,
        booking_time,
        status
      )
  ...
  const bookingColumnNames = new Set(
    bookingColumns.map(column => column.COLUMN_NAME)
  );
  ...
  // Friday is fixed weekly day off
  await pool.query(
    "UPDATE settings SET is_open=FALSE WHERE day_of_week=5"
  );
  ...
  if (!admins.length) {
    ...
    console.log(
      `Admin created: ${username} / ${password}`
    );
  }
}
...
app.get(
  ...
  async (req, res) => {
    ...
    const [rows] =
      await pool.query(
        "SELECT * FROM settings ORDER BY day_of_week"
      );
  ...
  async (req, res) => {
    ...
    if (existing.length) {
      ...
      return res
        ...
        .json({
          message:
            "This time slot is already requested or booked."
        });
    }
...
/* =========================================================
   Admin: Update Day Settings
========================================================= */

app.patch(
  "/api/admin/settings/:day",
  auth,
  async (req, res) => {
    ...
    if (day === 5) {
      ...
      return res
        ...
        .json({
          message:
            "Friday is a fixed weekly day off."
        });
    }

    if (
      !validScheduleTime(start_time) ||
      !validScheduleTime(end_time) ||
      String(start_time).slice(0, 5) >=
        String(end_time).slice(0, 5)
    ) {
    ...
    if (!Array.isArray(closed_periods)) {

      return res
        .status(400)
        .json({
          message:
            "Invalid closed periods."
        });
    }
...
initDb()
  ...
  .catch(err => {
    ...
    console.error(
      "Create the database first if needed: CREATE DATABASE baraa_aesthetics;"
    );
  });